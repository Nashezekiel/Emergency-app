...
This guide covers server-side features including authentication, Firebase Firestore database, tRPC API, and integrations. **Only read this if your app needs these capabilities.**
...
| Data syncs across devices | Yes | Yes | Firebase Firestore + tRPC |
| User accounts / login | Yes | Yes | Manus OAuth |
| AI-powered features | Yes | **Optional** | LLM Integration |
| User uploads files | Yes | **Optional** | Firebase Storage |
| Server-side validation | Yes | **Optional** | tRPC procedures |
| Real-time emergency updates | Yes | **Optional** | Firebase Real-time Listeners |
...
```
server/
  firebase.ts        ← Firebase Firestore & Storage setup
  routers.ts         ← tRPC procedures (add API routes here)
  storage.ts         ← Firebase Storage helpers (can extend)
...
Only touch to files with "←" markers. Anything under `_core/` directories is framework-level—avoid editing unless you are extending infrastructure.
...
- **Real-time capabilities** - Essential for emergency response updates
- **Mobile-first** - Optimized for React Native integration
- **Scalable** - Handles growth from local to national deployment
- **Offline support** - Built-in caching and sync
- **Security** - Granular security rules and user permissions

### Collections Overview
- `users` - User profiles and authentication
- `incidents` - Emergency reports and status tracking
- `notifications` - User notifications and alerts
- Subcollections for media, notes, and status history

See `docs/firebase-schema.md` for complete schema details.
